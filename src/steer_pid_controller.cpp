...
{ // utility
  ...
  void reset_controller_reference_msg(
      const std::shared_ptr<ControllerCommandMsg> &msg, const std::vector<std::string> &ref_dof_names)
  {
...
namespace steer_pid_controller
{
  ...
  controller_interface::CallbackReturn SteerPidController::on_cleanup(
  ...
  controller_interface::CallbackReturn SteerPidController::on_configure(
      const rclcpp_lifecycle::State & /*previous_state*/)
  {
    ...
    ref_subscriber_ = get_node()->create_subscription<ControllerReferenceMsg>(
        ...
        std::bind(&SteerPidController::reference_callback, this, std::placeholders::_1));
  ...
  void SteerPidController::reference_callback(const std::shared_ptr<ControllerReferenceMsg> msg)
  {
    if (msg->dof_names.empty() && msg->values.size() == ref_dof_names_.size())
    {
      ...
      input_ref_.writeFromNonRT(ref_msg);
        ...
        msg->dof_names.size() == ref_dof_names_.size() &&
        msg->values.size() == ref_dof_names_.size())
    {
  ...
  std::vector<hardware_interface::CommandInterface> SteerPidController::on_export_reference_interfaces()
  {
    ...
    for (std::size_t i = 0; i < ref_dof_names_.size(); i++)
    {
      reference_interfaces.push_back(hardware_interface::CommandInterface(get_node()->get_name(),
                                                                          ref_dof_names_[i] + "/" + hardware_interface::HW_IF_POSITION,
  ...
  controller_interface::CallbackReturn SteerPidController::on_activate(
      const rclcpp_lifecycle::State & /*previous_state*/)
  {
  ...
  controller_interface::CallbackReturn SteerPidController::on_deactivate(
      const rclcpp_lifecycle::State & /*previous_state*/)
  {
    return controller_interface::CallbackReturn::SUCCESS;
  }
  ...
  controller_interface::return_type SteerPidController::update_and_write_commands(
...
PLUGINLIB_EXPORT_CLASS(
    steer_pid_controller::SteerPidController, controller_interface::ChainableControllerInterface)
